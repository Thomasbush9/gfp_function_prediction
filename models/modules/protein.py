...
from .pdb_parser import (
    load_and_fix_pdb_data,
    parse_mmcif_coordinates,
...
class Protein:
    ...
    def _parse_input(self, inp_obj):
        ...
        if isinstance(inp_obj, (str, Path)):
            ...
            ### Read text coordinate file
                ...
                try:
                    print(
                        f"WARNING! Ambiguity in file format {ext}. Attempting to read coordinates."
                    )
                    self.coord = np.loadtxt(
                        inp_obj, delimiter=self.delimiter, skip_rows=self.skip_rows
                    )
    ...
    def _update_nan_coords(self):
        ...
        if self.max_bfactor > 0.0:
            bfactor_zscore = (self.bfactor - np.nanmean(self.bfactor)) / np.nanstd(
                self.bfactor
            )
            self.coord[
                (bfactor_zscore > self.max_bfactor) | (np.isnan(self.bfactor))
            ] = np.nan
    ...
    def get_local_neighborhood(self):
        ...
        if not isinstance(self.dist_mat, np.ndarray):
            self._get_dist_mat()
        self.neigh_idx = [
            np.where((d > 0) & (d <= self.neigh_cut) & (np.isfinite(d)))[0]
            for d in self.dist_mat
        ]
...
class AverageProtein:
    ...
    def _parse_input(self, inp_obj, **kwargs):
        ...
        if not isinstance(inp_obj, (list, np.ndarray)):
            raise Exception(
                f"Input object type {type(inp_obj)} is not supported. Acceptable types:"
        ...
        if self.average_plddt:
            try:
                self.plddt = np.mean(
                    [protein.plddt for protein in self.proteins], axis=0
                )
            except TypeError:
                self.plddt = None
        else:
        ...
        if self.average_bfactor:
            try:
                self.bfactor = np.mean(
                    [protein.bfactor for protein in self.proteins], axis=0
                )
            except TypeError:
                self.bfactor = None
        else:
        ...
        if self.average_dist_mat:
            self.dist_mat = np.mean(
                [protein.dist_mat for protein in self.proteins], axis=0
            )
        else:
    ...
    def _get_local_neighborhood(self):
        ...
        for protein in self.proteins:
            ...
            if len(self.neigh_idx) == 0:
                kwargs = {
                    a: getattr(self, a)
                    for a in ["min_plddt", "max_bfactor", "neigh_cut"]
                }
    ...
    def _consolidate_neighbor_lists(self):
        ...
        for i in range(self.seq_len):
            ...
            idx_count = Counter(
                j for protein in self.proteins for j in protein.neigh_idx[i]
            )
            # Only include indices that are in all neighbor lists
            self.neigh_idx.append(
                [j for j, count in idx_count.items() if count == self.num_repeat]
            )
    ...
    def _rotate_and_average_neighbor_tensors(self):
        ...
        for i in range(self.seq_len):
            ...
            for j in range(self.num_repeat):
                ...
                idx = [
                    i0
                    for i0, j0 in enumerate(self.proteins[j].neigh_idx[i])
                    if j0 in self.neigh_idx[i]
                ]
                ...
                if not j:
                    ...
                    rotated_tensor = rotate_points(
                        self.proteins[j].neigh_tensor[i][idx], tensor[0]
                    )
