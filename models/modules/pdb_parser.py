...
import numpy as np
import pandas as pd
from Bio import Align, Seq, SeqIO, SeqRecord
...
def parse_mmcif_coordinates(path, chain=""):
    ...
    coord = np.array(
        [df[x].values for x in ["Cartn_x", "Cartn_y", "Cartn_z"]]
    ).T.astype(float)
...
def load_and_fix_pdb_data(path, chain=""):
    ...
    if len(seqres) < len(seq):
        raise Exception(
            "ERROR: the number of alpha-carbons exceeds the number of SEQRES residues!"
